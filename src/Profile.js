...
const projectId = '2JK2T6GEA0EuZCnE0JHVqNqoXRr';
const projectSecret = 'd42cb12b180e5da781983b15c5bdc226';
const auth =
...
const App = ({ contract }) => {
    ...
    const uploadToIPFS = async (event) => {
        ...
        if (typeof file !== 'undefined') {
            try {
                ...
                setAvatar(`https://decentrasocial.infura-ipfs.io/ipfs/${result.path}`)
            } catch (error) {
    ...
    const mintProfile = async (event) => {
        ...
        try {
            ...
            setLoading(true)
            await (await contract.mint(`https://decentrasocial.infura-ipfs.io/ipfs/${result.path}`)).wait()
            loadMyNFTs()
        } catch (error) {
